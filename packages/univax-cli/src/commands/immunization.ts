...
export default class CreateImmunization extends Command {
  ...
  static examples = ['$ immu immunization'];
  ...
  async run() {
    ...
    const questions: any[] = [
      {
        ...
        message: 'lotNumber',
        type: 'input',
        validate: (inp: string) => Boolean(inp.match(/\w{5,10}/g))
      },
      {
        type: 'list',
        name: 'route',
    ...
    const immunization = baseImmunization;
    ...
    const credential = await issuer.issueCredential(subjectDid, claim)
